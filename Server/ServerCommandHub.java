...
public class ServerCommandHub {
    ...
    public void add(Set<Organization> collection) throws IOException, ClassNotFoundException, SQLException {
        ...
        String sql = "INSERT INTO maksimus_organizations " + "(id, name, creationdate, annualturnover, emploeescount, type, officialaddress, userid, coordinates)"+
                "VALUES (NEXTVAL('ORGANIZATIONS_MAKSIMUS_SEQUENCE'), '" + wrap.getName() + "', '" + wrap.getCreationDate() + "', " + wrap.getAnnualTurnover() +
        ...
        ResultSet rs = stmt.executeQuery( "SELECT * FROM MAKSIMUS_ORGANIZATIONS WHERE ID=CURRVAL('ORGANIZATIONS_MAKSIMUS_SEQUENCE')" );
        while ( rs.next() ) {
            int id = rs.getInt("id");
    ...
    public void update_by_id(Set<Organization> collection, long id) throws IOException, ClassNotFoundException, SQLException {
        ...
        if (stream.count()==1) {
            ...
            String sql =
                    "UPDATE maksimus_organizations SET" + "(name, creationdate, annualturnover, emploeescount, type, officialaddress, coordinates)"+
    ...
    public void remove_by_id(long id, Set<Organization> collection) throws IOException, SQLException {
        ...
        if (stream.count()==1) {
            ...
            String sql = "DELETE FROM MAKSIMUS_ORGANIZATIONS WHERE ID="+ id+";";
            Statement strm = this.connection.createStatement();
            strm.execute(sql);
            connection.commit();
    ...
    public void clear(Set<Organization> collection) throws IOException, SQLException {
        ...
        if (collection.stream().filter(o-> o.getUSERID()==userid).count()>0) {
            ...
            String sql = "DELETE FROM MAKSIMUS_ORGANIZATIONS WHERE USERID="+ userid+";";
            Statement strm = this.connection.createStatement();
            strm.execute(sql);
            connection.commit();
    ...
    public void remove_greater(Set<Organization> collection, String name) throws IOException, SQLException {
        ...
        if (collection.stream().filter(o-> o.getName().toLowerCase().equals(name)).count()>0){
            ...
            {
                collection.remove((Organization) o);
                String sql = "DELETE FROM MAKSIMUS_ORGANIZATIONS WHERE ID="+ ((Organization) o).getId()+";";
                Statement strm = this.connection.createStatement();
                strm.execute(sql);
                connection.commit();
            }

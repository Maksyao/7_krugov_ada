...
//            logger.info("Start messages exchange");
    ...
    public int login(String login, String password_hashed) throws SQLException {
        PreparedStatement strm = this.connection.prepareStatement("SELECT * FROM MAKSIMUS_USERS WHERE login=? AND password =?");
    ...
    public int reg(String login, String password_hashed) throws SQLException {
            try{
                PreparedStatement strm = this.connection.prepareStatement("SELECT * FROM MAKSIMUS_USERS WHERE login=?");
                strm.setString(1, login);
                ResultSet rs = strm.executeQuery();
                connection.commit();
                while (rs.next())
            ...
            PreparedStatement strm  = this.connection.prepareStatement("INSERT INTO MAKSIMUS_USERS (login, password, id) VALUES (?, ?, nextval('users_maksimus_sequence'))");
            strm.setString(1, login);
            strm.setString(2, password_hashed);
            strm.execute();
            connection.commit();
